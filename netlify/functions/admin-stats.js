...
exports.handler = async (event, context) => {
  ...
  const url = `${SUPABASE_URL}/rest/v1/events?select=site_id,session_id,user_id,ip_hash,user_agent,referrer,url,path,event_type,ts&site_id=eq.${
  ...
  const topReferrers = groupBy(events, (e) => e.referrer || "(direct)").slice(
    0,
    20
  );
  const byDevice = groupBy(events, (e) =>
